...
for( i in  1:PARAM$modelos_qty )
{
  ...
  if (PARAM$scale_undersampling) {
        scale_factor <- 1 / PARAM$scale_undersampling
        rescale <- function(pname) {
                parametros[[pname]] <- parametros[[pname]] * scale_factor
        }
        rescale("min_data_in_leaf")
  }
